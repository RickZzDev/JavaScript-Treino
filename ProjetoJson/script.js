...
//Chamando os elementos HTMl para o JS
var $divHtml = document.getElementById('seguraLinhas');
var $btnPesquisar = document.getElementById('btnPesquisar');
var $txtEstado = document.getElementById('txtEstado');



$btnPesquisar.addEventListener("click",()=>zeraHtml());
$btnPesquisar.addEventListener("click",()=>loopFunc());
$btnPesquisar.addEventListener("click",()=>confere());



//Decalarando a variavel que guardara o que for digitado
var digitado = "";


//Função que confere se foi digitado algo na caixa
const confere =()=> {
    if($txtEstado.value ==""){
        alert("favor digitar a sigla do estado")
    }
}

//ESTA FUNÇÃO VERIFICA SE A DIV DO HTML JA POSSUI ALGO ESCRITO NELA, SE ELA JA ESTIVER COM ALGO ESCRITO, A FUNÇÃO IRA SUBSTITUIR POR NADA, OU SEJA, ZERAR O HTML PARA QUE NÃO HAJA O ACUMULO DESNECESSARIO E CONFUSÃO DE INFORMAÇÕES
const zeraHtml = () =>{
    if(!$divHtml ==""){
        $divHtml.innerHTML = "";
    }
...
//ESSA FUNÇÃO ENVIA PARA O HTML A DIV COM O INDICE CONTADOR E COM A CIDADE DO ESTADO DESEJADO
    var exibirArray = (cidade,cont,tamanho) => {
        for (let a =0;a <1;a++){
            if(cont % 2 == 0){
             
             teste = `
                <div class="linhasTeste">
                    <div class="numeracao">
                        ${cont + 1}
                    </div>
                    <div class='nomeCidade2'>
                        ${cidade}
                    </div>
               </div>`        
           return teste;      
...
// ESTE LOOP É O QUE ATIVA A FUNÇÃO DETERMINADAS VEZES DE ACORDO COM O TAMANHO DO ARRAY
    
let loopFunc = () =>{
        
        digitado = $txtEstado.value.toUpperCase();
    
        //FUNÇÃO PARA FILTRAR O ESTADO DESEJADO
        const filtrar = (json) => json.sigla==digitado;
    
        
        //FILTRO
        const filtrado = json.filter(filtrar);
        
        
        
        //TAMANHO DO ARRAY CONTENDO AS CIDADES DO DETERMINADO ESTADO
        let tamanho = filtrado[0].cidades.length;
    
    
    
        //loop que chama a função exibirArray() varias vezes de acordo com o numero de cidades e além disse envia para a função um determinado contador
        for( let b = 0; b< tamanho; b++){
        contador = b;
        console.log(contador);
        $divHtml.innerHTML += exibirArray(filtrado[0].cidades[b].nome, contador,tamanho);
