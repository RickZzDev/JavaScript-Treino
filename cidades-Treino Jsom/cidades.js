...
const buscarCidades = (db, uf ) => {
   return db.filter(estado => estado.sigla == uf)[0].cidades.map(cidades => cidades.nome);
   
   
}

const mostrarCidades = () =>{
   const estado2 = document.querySelector('#estado').value;
   const cidades1 = buscarCidades(cidades,estado2.toUpperCase());
   const $container = document.querySelector('#results');
   $container.innerHTML = criarLinhas(cidades1);
}

const criarLinhas = (cidades) => {
   return cidades.reduce( (acc,cidades,i) => 
      `${acc}
      <tr>
         <td>${i} </td>
         <td> ${cidades}</td>
